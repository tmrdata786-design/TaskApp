...
export default function EditTaskModal({ taskId, onClose }: { taskId: string, onClose: () => void }) {
  ...
  if (loading) return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-[#11141A] p-6 rounded-2xl border border-[#1F2937]">
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-[#11141A] p-6 rounded-2xl border border-[#1F2937] w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <select value={form.area} onChange={e => handleUpdate('area', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white">
                {areas.map(a => <option key={a.id} value={a.name}>{a.name}</option>)}
              </select>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-500">Task Type</label>
              <select value={form.task_type} onChange={e => handleUpdate('task_type', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <select value={form.project || ''} onChange={e => handleUpdate('project', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white">
            ...
            <div className="space-y-1.5">
              ...
              <select value={form.assignee} onChange={e => handleUpdate('assignee', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              ...
              <select value={form.priority} onChange={e => handleUpdate('priority', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white">
                {['High', 'Medium', 'Low'].map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-500">Start Date</label>
              <input type="date" value={form.start_date || ''} onChange={e => handleUpdate('start_date', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-500">End Date</label>
              <input type="date" value={form.end_date || ''} onChange={e => handleUpdate('end_date', e.target.value)} className="w-full px-3 py-2 bg-[#0B0D10] border border-[#1F2937] rounded-lg text-white" />
            </div>
          </div>
